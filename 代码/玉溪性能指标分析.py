...
import analyze_yuxi_merged_cleaned_verified as base
from exclude_ai_error_from_raw import ROOT, ensure_unique_dir


OUTPUT_ROOT_NAME = "玉溪结果"
BOOTSTRAP_REPS = 500
...
BODY_GROUPS = ["头部", "胸部", "腹部", "脊柱"]
BODY_DISPLAY = {"头部": "Head", "胸部": "Chest", "腹部": "Abdomen", "脊柱": "Spine"}
TYPE_GROUPS = {
...
def configure_plot_style() -> None:
    ...
    for preferred in ["Arial", "Microsoft YaHei", "SimHei", "DejaVu Sans"]:
...
def build_modality_outputs(instance_records) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
...
def build_body_outputs(instance_records) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
    summary_rows: list[dict[str, object]] = []
    per_class_rows: list[dict[str, object]] = []
    count_map: dict[str, pd.DataFrame] = {}
    norm_map: dict[str, pd.DataFrame] = {}
...
def build_type_outputs(instance_records) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
    summary_rows: list[dict[str, object]] = []
    per_class_rows: list[dict[str, object]] = []
    count_map: dict[str, pd.DataFrame] = {}
    norm_map: dict[str, pd.DataFrame] = {}

    for idx, (group_name, config) in enumerate(TYPE_GROUPS.items(), start=200):
...
def plot_summary_panel(summary_df: pd.DataFrame, groups: list[str], title: str, output_png: Path, output_pdf: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(max(8.5, len(groups) * 1.6), 5.8))
    for idx, group in enumerate(groups):
        ...
        bars = ax.bar(
            ...
            edgecolor="black",
            linewidth=0.4,
            label=BODY_DISPLAY.get(group, group),
        )
...
def main() -> None:
    if not base.SOURCE_DIR.exists():
        raise FileNotFoundError(f"Missing directory: {base.SOURCE_DIR}")

    configure_plot_style()
    output_root = ensure_unique_dir(ROOT / OUTPUT_ROOT_NAME)
    output_root.mkdir(parents=True, exist_ok=False)

    _, _, report_records, instance_records, file_stats_df = base.load_records()
    ...
    with pd.ExcelWriter(workbook_path, engine="openpyxl") as writer:
        pd.DataFrame(
            [
                {"Item": "Source directory", "Value": str(base.SOURCE_DIR)},
        ...
        for group, df in modality_counts.items():
            df.to_excel(writer, sheet_name=f"{group.replace('X-ray','Xray')}_cm_count"[:31])
            modality_norm[group].to_excel(writer, sheet_name=f"{group.replace('X-ray','Xray')}_cm_norm"[:31])
        for group, df in body_counts.items():
            df.to_excel(writer, sheet_name=f"{group}_cm_count"[:31])
            body_norm[group].to_excel(writer, sheet_name=f"{group}_cm_norm"[:31])
    ...
    plot_summary_panel(
        modality_summary_df,
        list(MODALITY_GROUPS),
        "Exact-Label Main Metrics by Modality",
        output_root / "SCI_modality_exact_main_metrics.png",
        output_root / "SCI_modality_exact_main_metrics.pdf",
    )
    plot_summary_panel(
        body_summary_df,
        BODY_GROUPS,
        "Exact-Label Main Metrics by Body Region",
        output_root / "SCI_bodypart_exact_main_metrics.png",
        output_root / "SCI_bodypart_exact_main_metrics.pdf",
    )
    plot_summary_panel(
        type_summary_df,
        list(TYPE_GROUPS),
        "Exact-Label Main Metrics by Type Group",
        output_root / "SCI_type_exact_main_metrics.png",
        output_root / "SCI_type_exact_main_metrics.pdf",
    )

    plot_confusion_grid(
        modality_norm,
        list(MODALITY_GROUPS),
        {group: group for group in MODALITY_GROUPS},
        output_root / "SCI_modality_confusion_matrices.png",
        output_root / "SCI_modality_confusion_matrices.pdf",
    )
    plot_confusion_grid(
        body_norm,
        BODY_GROUPS,
        {group: BODY_DISPLAY[group] for group in BODY_GROUPS},
        output_root / "SCI_bodypart_confusion_matrices.png",
        output_root / "SCI_bodypart_confusion_matrices.pdf",
    )
    plot_confusion_grid(
        type_norm,
        list(TYPE_GROUPS),
        {group: group for group in TYPE_GROUPS},
        output_root / "SCI_type_confusion_matrices.png",
        output_root / "SCI_type_confusion_matrices.pdf",
    )
